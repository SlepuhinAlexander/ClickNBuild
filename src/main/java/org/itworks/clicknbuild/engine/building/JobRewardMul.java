...
public final class JobRewardMul extends BuildingAttribute {
    @Override
    protected void initValues(BuildingType type, int level) {
        pack.clear();
        ResStatPack stats = ResCalculator.mul(type.stats.getStats()[level - 1].get(BuildingAttrType.JOB_REWARD_MUL),
