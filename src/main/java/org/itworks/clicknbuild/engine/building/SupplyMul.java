...
public final class SupplyMul extends BuildingAttribute {
    @Override
    protected void initValues(BuildingType type, int level) {
        pack.clear();
        ResStatPack stats = ResCalculator.mul(type.stats.getStats()[level - 1].get(BuildingAttrType.SUPPLY_MUL),
    ...
    protected void applyProductivity(double productivity) {
        ...
        pack.forEach((type, resChunk) -> resChunk.setCurrent(resChunk.getMax() * clamped / 100d));
    }
}
